...
app.whenReady().then(() => {
  ...
  ipcMain.handle('save-node-position', (_, id, x, y) => db.saveNodePosition(id, x, y))
  ipcMain.handle('get-clusters', () => db.getClusters())
  ipcMain.handle('create-cluster', (_, name, color) => db.createCluster(name, color))
  ipcMain.handle('update-cluster', (_, id, name, color) => db.updateCluster(id, name, color))
  ipcMain.handle('delete-cluster', (_, id) => db.deleteCluster(id))
  ipcMain.handle('set-topic-cluster', (_, topicId, clusterId) => db.setTopicCluster(topicId, clusterId))
